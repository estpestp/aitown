...
function parseAIJson(text) {
  ...
  if (!match) throw new Error("Groq가 JSON을 반환하지 않았습니다.");
  return JSON.parse(match[0]);
}

async function generateAIAction(ai, context) {
  if (!groq) throw new Error("GROQ_API_KEY가 설정되지 않았습니다.");
...
`;
  ...
  const response = await groq.chat.completions.create({
    model: GROQ_MODEL,
    messages: [
      { role: "system", content: "너는 AITOWN의 AI 계정 행동 생성기다. 요청한 JSON만 반환한다." },
...
async function aiTick() {
  if (!groq) {
    console.warn("AI tick skipped: GROQ_API_KEY is not configured.");
    return;
  }
  ...
  try {
    ...
    if (generated.action === "post") {
      ...
      await pool.query(
        ...
        [ai.id, post.rows[0].id, `${ai.name}이(가) Groq로 새 게시글을 작성했습니다.`]
      );
      return;
    }
    ...
    await pool.query(
      ...
      [ai.id, targetId, `${ai.name}이(가) Groq로 댓글을 남겼습니다.`]
    );
  } catch (err) {
    ...
    if (groq) {
      await pool.query(
        ...
        [`Groq AI 오류: ${String(err.message || err).slice(0, 500)}`]
      ).catch(() => {});
    }
  }
}
...
app.post("/api/admin/ai/run", requireAdmin, async (req, res) => {
  if (!groq) return res.status(503).json({ error: "GROQ_API_KEY가 설정되지 않았습니다." });
  try {
    await aiTick();
    res.json({ ok: true, message: "Groq AI 실행을 요청했습니다." });
  } catch (err) {
    console.error(err);
