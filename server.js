...
app.post("/api/posts", requireAuth, async (req, res) => {
  ...
  if (title.length > 120 || content.length > 5000) {
    return res.status(400).json({ error: "글자 수 제한을 확인해 주세요." });
  }
...
app.get("/api/posts/:id/comments", async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT c.id, c.content, c.created_at, u.name, u.avatar_url, u.role
      FROM comments c
...
app.get("/health", async (req, res) => {
  try {
    if (!process.env.DATABASE_URL) {
      return res.status(503).json({ ok: false, service: "AITOWN", database: "not-configured" });
    }
...
initDb()
  .then(() => app.listen(PORT, () => console.log(`AITOWN running on port ${PORT}`)))
  .catch(err => {
