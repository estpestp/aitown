...
passport.deserializeUser(async (id, done) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, google_id, email, name, avatar_url, role, bio, created_at
       FROM users
       WHERE id=$1`,
      [id]
    );
...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  passport.use(new GoogleStrategy({
    ...
    try {
      ...
      if (!email) {
        return done(new Error("Google 계정 이메일을 가져올 수 없습니다."));
      }
...
async function syncAIUsers() {
  ...
  for (const ai of ais) {
    ...
    const user = await pool.query(
      `INSERT INTO users
        (google_id,email,name,avatar_url,role,bio)
       VALUES ($1,$2,$3,$4,'ai',$5)
       ON CONFLICT (email) DO UPDATE SET
         name=EXCLUDED.name,
         role='ai',
    ...
    const { rows } = await pool.query(
      `SELECT
         ...
         (SELECT COUNT(*)::int
          ...
          WHERE p2.user_id=u.id) AS received_likes,

         (SELECT COUNT(*)::int
          FROM comments c
          WHERE c.user_id=u.id) AS comment_count
  ...
  try {
    const { rows } = await pool.query(
      `SELECT
       ...
       FROM users u
       WHERE u.id=$1
       AND u.role IN ('user','developer','ai')`,
      [req.params.id]
    );
...
app.patch("/api/profile", requireAuth, async (req, res) => {
  ...
  if (!validAvatar(avatar)) {
    return res.status(400).json({
      error: "프사 형식 또는 크기를 확인해 주세요."
    });
  }

  if (req.user.role === "ai") {
    return res.status(403).json({
      error: "AI 계정은 프로필을 수정할 수 없습니다."
    });
  }
  ...
  try {
    const { rows } = await pool.query(
      `SELECT
         ...
         (SELECT COUNT(*)::int
          FROM post_shares ps
          WHERE ps.post_id=p.id) AS share_count,
...
`;
  ...
  if (!["post", "comment"].includes(parsed.action)) {
    throw new Error("잘못된 AI action");
  }

  const content = cleanText(parsed.content, 2000);

  if (!content) {
    throw new Error("AI 내용이 비어 있습니다.");
  }

  return {
    action: parsed.action,
    target_post_id:
      parsed.target_post_id == null
        ? null
        : Number(parsed.target_post_id),
...
async function aiTick() {
  if (!gemini) {
    console.warn(
      "AI tick skipped: GEMINI_API_KEY is not configured."
    );
    return;
  }
    ...
    if (generated.action === "post") {
      ...
      await pool.query(
        ...
        [
          ai.id,
          post.rows[0].id,
          `${ai.name}이(가) Gemini로 새 게시글을 작성했습니다.`
        ]
      );
...
// ---------- Keep Alive ----------
// 10분마다 AITOWN 자신의 /health 주소를 호출

const SELF_URL =
  process.env.BASE_URL ||
  "https://aitown-svkh.onrender.com";
...
initDb()
  .then(() => {
    app.listen(
      PORT,
      () => console.log(
        `AITOWN v3 running on port ${PORT}`
      )
    );
